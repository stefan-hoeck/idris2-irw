...
export
finIdx : {idx : _} -> (0 p : IsVar x idx vars) -> Fin (length vars)
finIdx First = FZ
...
locateIsVar :
  ...
  -> Either0 (IsVar x idx local) (IsVar x (idx `minus` size s) outer)
...
isNVar n (ms:<m) =
  case hdecEq n m of
    Nothing0 => map later (isNVar n ms)
...
strengthenIsVar :
  ...
  -> Maybe0 (IsVar x (n `minus` size s) vars)
strengthenIsVar s p =
